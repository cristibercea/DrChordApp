...
async def request_signup(user_data: RegisterRequest, background_tasks: BackgroundTasks):
    if await user_repo.get_by_email(user_data.email): raise HTTPException(status_code=400, detail="Email already in use.")
...
async def update_profile(req: UpdateProfileRequest, user_id: int = Depends(extract_user_from_header)):
    updated_user_obj = await user_service.update_user(
        user_id=user_id, name=req.name, current_password=req.current_password, new_password=req.new_password
    )
    return {
        "message": "Profile updated successfully",
        "name": updated_user_obj.get_name(),
        "email": updated_user_obj.get_email()
    }
